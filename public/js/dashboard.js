window.addEventListener("DOMContentLoaded", (event) => {
  ...
  const updateLangDefault = document.getElementById("updateLangDefault");
  ...
  const btnManagerLang = document.getElementById("manageLang")

  uploadSection.style.display = "block";
  listProductSection.style.display = "none";
  addProductSection.style.display = "none";
  detailProductSection.style.display = "none";
  updateLangDefault.style.display = "none";
  ...
  btnManagerLang.addEventListener("click", function () {
    uploadSection.style.display = "none";
    listProductSection.style.display = "none";
    addProductSection.style.display = "none";
    detailProductSection.style.display = "none";
    updateLangDefault.style.display = "block";
  });
    ...
    productLinks.forEach(link => {
      link.addEventListener('click', function () {
        ...
        fetch("/api//products/detail/admin?id=" + id)
          .then((response) => response.json())
          .then(async (product) => {
            const dataItem = await product.data;
  ...
  languageSelect.addEventListener('change', updateLanguageBackground);
  updateLanguageBackground();
  //  update lang
  const updateLangButton = document.getElementById('UpdateLangBTN');
  updateLangButton.addEventListener('click', () => {
    const lang = languageSelect.value;
    fetch('api/lang/update', {
      method: "PUT",
      headers: {
        "Authorization": `Bearer ${token}`,
      ...
      .catch(error => {
        alert('Có lỗi xảy ra khi cập nhật ngôn ngữ.' + error);
      });
  });
});
